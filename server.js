// Instale o pacote WebSocket com: npm install ws
...
wss.on('connection', (ws) => {
  if (players.length < 2) {
    ...
    if (players.length === 2) {
      players.forEach((player, index) => {
        player.send(JSON.stringify({ type: 'status', message: `Jogadores conectados. Você é o Jogador ${index + 1}.` }));
      });
    }
  } else {
    ...
    if (message.type === 'choice') {
      ...
      if (Object.keys(choices).length === 2) {
        ...
        players.forEach((player) => {
          player.send(JSON.stringify({ type: 'result', message: result }));
        });
  ...
  ws.on('close', () => {
    ...
    choices = {};
    players.forEach((player) => {
      player.send(JSON.stringify({ type: 'status', message: 'O outro jogador saiu. Aguardando novo jogador...' }));
    });
  });
});